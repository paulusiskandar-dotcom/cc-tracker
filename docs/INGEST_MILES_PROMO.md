...
pengayaan: pengecualian_utilitas, pengecualian_cicilan, pengecualian_asuransi, pengecualian_qris, pengecualian_pajak, pengecualian_ewallet_topup, pengecualian_spbu, pengecualian_pendidikan, pengecualian_virtual_account, pengecualian_paper (Paper.id / platform tagihan bisnis), pengecualian_emoney_topup (isi saldo Flazz/e-money/Brizzi/TapCash), pengecualian_bayar_ewallet (kartu ditautkan ke DANA/GoPay/ShopeePay lalu dipakai bayar merchant atau QRIS), paper_via_blibli_tokopedia (masing-masing hanya `tidak_dapat` | `terbatas` | `dapat` | `tidak_disebut`; nilai lain ditolak database), pengecualian_kutipan (JSON `{kategori: {kutipan, sumber_url, per_tanggal}}`, kunci kategori tanpa awalan `pengecualian_`), kelipatan_transaksi_rp (angka), batas_perolehan_bulanan, biaya_konversi_rp (angka), iuran_tahunan_utama_rp (angka), kelipatan_hasil, batas_perolehan_bulanan_angka (angka), batas_perolehan_bulanan_satuan, batas_perolehan_bulanan_kategori, batas_konversi_angka (angka), batas_konversi_satuan, batas_konversi_periode, min_konversi_angka (angka), min_konversi_satuan, poin_per_mile_kf (angka: berapa poin bank untuk 1 KrisFlyer mile menurut S&K resmi; kosongkan kalau poin tidak bisa ditukar ke KrisFlyer), iuran_hapus_status, iuran_hapus_syarat, angka_kutipan (JSON), sumber_resmi_dibaca (JSON), catatan_resmi, asal_resmi, resmi_per_tanggal
...
pengayaan: jenis_produk, bank_penerbit_kartu, kartu_berlaku (JSON **array** nama kartu, mis. `["Danamon JCB Precious"]`; teks biasa ditolak), jenis_reward, program, program_reward, klasifikasi_oleh, butuh_status_prioritas (boolean), detail_kosong (boolean)
