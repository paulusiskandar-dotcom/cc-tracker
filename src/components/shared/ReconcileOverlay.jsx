...
export function useReconcile({ user, accountId, fromDate, toDate, ledgerRows, currentAccountId, accounts = [], merchantMaps = [] }) {
  ...
  const exitReconcile = useCallback(async () => {
    ...
    // CC installments: advance paid_months from the statement's "… : X/N"
    // markers (shared helper — same sync runs in Reconcile.jsx finalize).
    if (recAcc?.type === "credit_card") {
      try {
        const synced = await installmentsApi.syncFromStatementRows(user.id, accountId, stmtRows);
