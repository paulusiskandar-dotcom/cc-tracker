...
function GiveLoanCell({ r, onUpdate, T, accounts, employeeLoans }) {
  ...
  return (
    <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
      ...
      {isNew ? (
        <>
          <input style={{ ...inInp(T), flex: 1, minWidth: 80 }} placeholder="Name *"
            value={r.new_loan_name || ""} onChange={e => onUpdate({ new_loan_name: e.target.value })} />
          <input type="number" style={{ ...inInp(T), width: 46, flexShrink: 0 }} placeholder="Mo."
...
function BuyAssetCell({ r, onUpdate, T, accounts }) {
  ...
  return (
    <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
      ...
      {isNew ? (
        <>
          <input style={{ ...inInp(T), flex: 1, minWidth: 80 }} placeholder="Asset name *"
            value={r.new_asset_name || ""} onChange={e => onUpdate({ new_asset_name: e.target.value })} />
          <select style={{ ...inSel(T), width: 80, flexShrink: 0 }} value={r.new_asset_subtype || "Other"}
...
function TxHorizontalCard({
  ...
  const amtStr = isFX
    ? (source === "estatement" || fxWaiting
        // fxWaiting: IDR is unknown until the statement lands — show original currency only
