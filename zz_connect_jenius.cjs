...
const OPEN=16557342, GAP=2330; // OPEN = saldo 1 Jan 2026 (buku mulai 1 Jan; Des hanya penyambung)
...
// 1) 7 baris sisa → gadget
const{data:nul}=await supabase.from('ledger_staging').select('id,tx_date,amount,description').eq('user_id',uid).eq('account_id',J.id).eq('status','staged').is('tx_type',null);
for(const r of nul||[]){
  await supabase.from('ledger_staging').update({tx_type:'expense',category_id:cat('Electronics & Gadgets'),entity:'Personal',needs_review:false,
    description:((r.description||'')+' — gadget (Paulus 27/8)').slice(0,290)}).eq('id',r.id);
}
console.log('7 baris sisa → Electronics & Gadgets:',(nul||[]).length);
...
for(const r of st){
  const base={user_id:uid,tx_date:r.tx_date,amount:Number(r.amount),currency:'IDR',amount_idr:Number(r.amount),
    description:(r.description||'').slice(0,290),source:'backfill',notes:'backfill Jenius Des2025',
...
// baris koreksi celah genesis
rows.push({user_id:uid,tx_date:'2026-04-01',tx_type:'expense',amount:GAP,currency:'IDR',amount_idr:GAP,
  ...
  source:'backfill',notes:'backfill Jenius Des2025',from_type:'account',from_id:J.id,to_type:'expense',to_id:null,
...
for(const col of['from_id','to_id']){
  let off=0;for(;;off+=1000){
    const{data:c}=await supabase.from('ledger').select('amount').eq('user_id',uid).eq(col,J.id).range(off,off+999);
...
const proj=OPEN+exist+insNet, target=Number(J.current_balance);
console.log('aljabar:',rp(OPEN),'+ exist',rp(exist),'+ ins',rp(insNet),'=',rp(proj),'| target',rp(target),'| delta',rp(proj-target));
if(Math.abs(proj-target)>0.02){console.log('!! MELESET — BATAL');process.exit(1);}
...
want[J.id]=Number(F(J.id).current_balance);
const ids=[];
try{
  for(const r of rows){
