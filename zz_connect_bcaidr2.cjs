...
const OPEN=3801616.07, GAP=0; // saldo 1 Jan 2026 BCA IDR
...
for(const r of work){
  const base={user_id:uid,tx_date:r.tx_date,amount:Number(r.amount),currency:'IDR',amount_idr:Number(r.amount),
    description:(r.description||'').slice(0,290),source:'backfill',notes:'backfill BCA IDR',
...
if(GAP)rows.push({user_id:uid,tx_date:'2026-04-01',tx_type:'expense',amount:GAP,currency:'IDR',amount_idr:GAP,
  ...
  source:'backfill',notes:'backfill BCA IDR',from_type:'account',from_id:J.id,to_type:'expense',to_id:null,
...
console.log('saldo app sekarang',rp(J.current_balance),'→ KOREKSI ANCHOR',rp(proj-Number(J.current_balance)),'(nilai MERAH 3.349.998,12 yg dicari sejak awal)');
if(Math.abs(proj-Number(J.current_balance)-3349998.12)>0.02){console.log('!! koreksi tidak sesuai dugaan — BATAL');process.exit(1);}
